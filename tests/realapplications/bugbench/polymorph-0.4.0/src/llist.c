...
xtn_t *find_xtn(xtn_t *list, char *suspect){
  ...
  for(tmp=list;tmp!=NULL;tmp=(xtn_t *)tmp->next){
...
polym_t *find_dir(polym_t *list, char *suspect){
	...
	for(tmp=list;tmp!=NULL;tmp=(polym_t *)tmp->next){
...
xtn_t *find_last_xtn(xtn_t *target){
  ...
  for(run=target;run!=NULL;run=(xtn_t *)run->next){
...
polym_t *find_last_dir(polym_t *target){
  ...
  for(run=target;run!=NULL;run=(polym_t *)run->next){
