...
int main(int argc, char *argv[]){
  ...
  target = (char *)malloc(MAX);
  strcpy( target, "" );

  fprintf(stderr, "In the beginning of main, argc %d argv[0] %s argv[1] %s\n", argc, argv[0], argv[1]);
  grok_commandLine( argc, argv );

  //fprintf(stderr, "In the beginning of main, target %s\n", target);
  if( strlen(target) != 0 ){
...
void grok_commandLine(int argc, char *argv[]){
    ...
    fprintf(stderr, "grok_commandLine o is %c\n", o);
		switch( o ){
			case 'a':
				hidden = 1;
				break;
...
void convert_fileName(char *original){
  ...
  fprintf(stderr, "convert_filename original %s\n", original); 
	error = 0;
	strcpy( newname, "" );
