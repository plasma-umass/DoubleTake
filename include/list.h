...
// Initialize a node
inline void nodeInit(list_t * node) {
  node->next = node->prev = node;
}

inline void listInit(list_t * node) {
  nodeInit(node);
}

// Whether a list is empty
inline bool isListEmpty(list_t * head) {
  return (head->next == head);
}

// Next node of current node
inline list_t * nextEntry(list_t * cur) {
  return cur->next;
}

// Previous node of current node
inline list_t * prevEntry(list_t * cur) {
  return cur->prev;
}
...
inline void listInsertNode(list_t * node, list_t * prev) {
...
inline void listInsertHead(list_t * node, list_t * head) {
...
inline bool isListTail(list_t *node, list_t *head)
{
