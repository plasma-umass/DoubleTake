...
//    PRINF("***********DoubleTake: problematic address %p with value 0x%lx!!!!!\n", addr, value);
    if(_numWatchpoints < xdefines::MAX_WATCHPOINTS) {
      ...
      _numWatchpoints++;
  //    PRINF("***********CAUGHT: problematic address %p with value 0x%lx _numWatchpoints %d!!!!!\n", addr, value, _numWatchpoints);
    } 
  }
...
//      PRINF("CHILD installWatchpoints %d watchpoints %d!!!!!!!!!\n", __LINE__, _numWatchpoints);    
	...
	perror("DoubleTake error:");
     ...
     // PRINF("child install watchpoints now\n");
      ...
      {
        PRINF("child exit now!0\n");
        exit(-1);
      }
    }
    else {
      PRWRN("Can't fork when installing watch points, with error: %s\n", strerror(errno));
      while(1);
    }
  }
