...
int main(int argc, char **argv)
{
	...
	if(child == 0) {
		...
		printf("Child %d have receive the notification from parent and try to _exit now.\n", getpid());
		// Since parent have received the signal and notify me, then I can _exit successfully.
		_exit(0);
		...
		// Then I will wait for the child to _exit.
		wait(&status);
		printf("parent find out child has _exited\n");
	}
}
